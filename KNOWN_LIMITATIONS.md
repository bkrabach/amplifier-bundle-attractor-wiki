...
| **Dot-graph resolver in Amplifier Resolve** | ✅ Proven — validated live end-to-end through the Resolve dot-graph resolver (2026-06-24). A provisioning wrapper pipeline ran all 7 nodes to completion: `setup` (uv pip install wiki_attractor from Gitea, INSTALLED_OK), `init_wiki` (CLI found at `/opt/uv-tools/amplifier/bin/wiki-attractor v0.1.0`, `wiki-attractor init` succeeded, WIKI_INIT_OK), `stage_source` (SOURCE_STAGED), `ingest` (5.6 min, all 14 internal ingest nodes succeeded including mine/write_pages/reconcile/provenance_audit/weave/verify, INGEST_EXIT:0), `verify` (`verify: clean (4 entity page(s) checked)`, VERIFY_CLEAN), `report` (LLM box node ran successfully). The existing wiki-attractor `.dot` files are **unchanged** — a provisioning shim wrapper sets up the wiki environment before delegating to the wiki-attractor CLI. Provisioning recipe: `uv pip install --python /opt/uv-tools/amplifier/bin/python git+http://GITEA_URL/admin/amplifier-bundle-attractor-wiki`. Instance artifacts at `.amplifier/evaluation/wiki-attractor-dot-resolver/20260624T0024Z/`. Note: the full DTU-based `resolve-stack.sh` launch failed due to a separate `resolve-stack.yaml` provisioning bug (`amplifier-resolve resolver add` doesn't accept PEP 508 git URL format in the freshly-installed container version); the validation ran using the host-based `amplifier-resolve` with Incus workers, which faithfully reproduces the Resolve execution model. |
