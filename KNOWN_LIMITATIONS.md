...
| **Dot-graph resolver in Amplifier Resolve** | ✅ **Multi-trial native execution confirmed** — Three independent trials (2026-06-24), all on `amplifier-bundle-resolve @ e4cad01`. Each trial: submitted `ingest.dot` (unmodified, byte-for-byte) as `pipeline: "custom"` + `dot_content` to the dot-graph resolver v0.2.0; **all 14 ingest.dot nodes completed** natively (verified via Resolve API `/api/instances/{id}/state`); produced wiki **directly captured from worker container** via `incus file pull` and **independently run through `verify.sh`** — not inferred from gate exit codes. Results: Trial 1 (`ec045e94988a`) — 34 entity pages, `verify: clean (34 entity page(s) checked)`, 780s. Trial 2 (`8aba21e68745`) — 33 entity pages, `verify: clean (33 entity page(s) checked)`, 1140s. Trial 3 (`c860462aa670`) — 33 entity pages, `verify: clean (33 entity page(s) checked)`, 1125s. Entity types produced per trial: 4–5 outcomes, 16 concepts, 5 people, 8 sources. **Honest residual:** `done` (terminal Msquare exit) shows `pending` in all trials — expected: the pipeline terminates when `archive` completes, before the terminal exit node is explicitly executed. `wiki_attractor` package not installed in the worker — box nodes operate via wiki workspace files; scripts in `.wiki/scripts/` are self-contained stdlib-only Python. Eval artifacts: `.amplifier/evaluation/wiki-attractor-dot-resolver/20260624T0442Z-native-confirm/`. |
